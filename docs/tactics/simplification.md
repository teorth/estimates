...
Note: in a previous version we also used `sympy`'s native simplifier `simplify()`, but this caused additional issues due to various unwanted simplifications generated by this tool (most notably involving subtraction, which is not fully defined for orders of magnitude).  Also it does not guard against issues such as division by zero.  For instance, `x/x` will simplify to `1` even if `x` is not proven to be non-zero.  As such, we have currently disabled use of this simplifier in the code.
